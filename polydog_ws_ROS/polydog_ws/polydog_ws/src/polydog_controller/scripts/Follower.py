...
class Follower(GaitController):
    def __init__(self, default_stance, stance_time, swing_time, time_step, use_imu):
        ...
        z_error_constant = 0.02 * 5    # This constant determines how fast we move
        ...
        self.pid_controller = PID_controller(0.01, 0.01, 0.00)
        self.pid_controllerZ = PID_controllerZ(0.005, 0, 0)
    ...
    def follow(self, msg2, command):
        #print(msg2.data[1])
        if msg2.data[1] != 0:
            compensation = self.pid_controller.run(msg2.data[0], 0, 0)
            compensationz = self.pid_controllerZ.run(msg2.data[1])
            yaw_compensation = compensationz[0]
            distance_compensation = compensation[0]
        print(yaw_compensation)
        if compensationz[0] < 0 :
            command.yaw_rate = compensationz[0] * self.max_yaw_rate * 1.5
        if compensationz[0] >= 0 :
            command.yaw_rate = compensationz[0] * self.max_yaw_rate 
        command.velocity[0] = -compensation[0] * self.max_x_velocity
    ...
    def step(self, state, command):
        ...
        if self.trotNeeded:
            ...
            if self.use_imu:
                ...
                #print(state.imu_roll)
                #print(state.imu_pitch)
                #print(state.imu_yaw)
                roll_compensation = 0 #-compensation[0]
                pitch_compensation = 0 #-compensation[1]
                yaw_compensation = 0#-compensation[2]
            ...
            return new_foot_locations
                ...
                #print(state.x)
               # compensation = self.pid_controller.run(0, 0, state.x)
                #print(state.imu_roll)
                #print(state.imu_pitch)
                #print(state.imu_yaw)
                #roll_compensation = 0 #-compensation[0]
