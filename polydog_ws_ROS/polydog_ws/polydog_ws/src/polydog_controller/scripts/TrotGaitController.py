...
class TrotGaitController(GaitController):
    def __init__(self, default_stance, stance_time, swing_time, time_step, use_imu):
        ...
        contact_phases = np.array([[1, 0, 1, 1],  # 0: Leg swing
                                   [1, 1, 1, 0],  # 1: Moving stance forward
                                   [1, 1, 1, 0],  
                                   [1, 0, 1, 1]])
        ...
        self.pid_controller = PID_controller(0.01, 0.00, 0.00)
        self.pid_controllerZ = PID_controllerZ(0, 0, 0)

    def updateStateCommand(self, msg, state, command):
        self.msg = msg
        command.velocity[0] = msg.axes[1] * self.max_x_velocity
        #command.velocity[1] = msg.axes[0] * self.max_y_velocity
        command.yaw_rate = msg.axes[3] * self.max_yaw_rate
        #if command.follow_event :
            #print(state.x)
            #compensation = self.pid_controller.run(0, 0, state.x)
            #yaw_compensation = compensation[2]
            #print(compensation[2])
            #command.yaw_rate = compensation[2] * self.max_yaw_rate
